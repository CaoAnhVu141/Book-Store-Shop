...
import { deletePayment, fetchListPayment, fetchPaymentById } from "@/services/api";
...
const TablePayment = () => {
    ...
    const [dataDetailPayment, setDataDetailPayment] = useState<IPayment | null>(null);

    const [openCreatePayment,setOpenCreatePayment] = useState<boolean>(false);
    ...
    const columns: ProColumns<IModelPaginate>[] = [
        ...
        },
        {
            ...
            render(dom, entity, index, action, schema) {
                return (
                    <a
                        onClick={async () => {
                            const response = await fetchPaymentById(entity._id);
                            if(response && response.data){
                                setOpenDetailPayment(true);
                                setDataDetailPayment(response.data);
                            }
                        }}
                        href="#">{entity._id}</a>
                )
        ...
        },
        {
            ...
            render: (status: boolean) => (
                status ? <span style={{ color: 'green' }}>Hoạt động</span> : <span style={{ color: 'red' }}>Không hoạt động</span>
            )
        },
        {
            disable: true,
            ...
            <>
                <ProTable<IModelPaginate, TSearch>
                    ...
                    request={async (params, sort, filter) => {
                        ...
                        const response = await fetchListPayment(query);
                        if (response && response.data) {
                ...
                />
                <DetailPayment
                openDetailPayment={openDetailPayment}
                setOpenDetailPayment={setOpenDetailPayment}
                dataDetailPayment={dataDetailPayment}
                setDataDetailPayment={setDataDetailPayment}
                />
                <CreatePayment
